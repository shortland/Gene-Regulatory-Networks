...
Network = () ->
  ...
  network.updateSearch = (searchTerm) ->
    ...
    node.each (d) ->
      ...
      if searchTerm.length > 0 and match >= 0
        ...
        d.searched = false
        element.style("fill", (d) -> nodeColors(d.origin))
  ...
  setupData = (data) ->
    ...
    circleRadius = d3.scale.sqrt().range([10, 12]).domain(countExtent)
    # was
  ...
  moveToRadialLayout = (alpha) ->
    k = alpha * 0.1
    (d) ->
      centerNode = groupCenters(d.origin)
  ...
  # particular node.
  strokeFor = (d) ->
    d3.rgb(nodeColors(d.origin)).darker().toString()
